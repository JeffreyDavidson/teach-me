    ...
    <div class="row">
        <div class="col-md-6">
            <x-input.group label="Term" for="term" :error="$errors->first('term')">
                <x-input.select id="term" name="term" :options="$terms" value="{{ old('term', $semester->exists ? $semester->term : '') }}" />
            </x-input.group>
        </div>
        <div class="col-md-6">
            <x-input.group label="Year" for="year" :error="$errors->first('year')">
                <x-input.select id="year" name="year" :options="$years" value="{{ old('year', $semester->exists ? $semester->year : '')  }}" />
            </x-input.group>
        </div>
    </div>
